...
if __name__ == '__main__':
    ...
    if args.command == "train":
        ...
        if args.dtype == "esrgan":
            ...
            detect_instance.run_on_folder(input_folder=src_path, output_folder=out_folder, is_video=False, is_mosaic=False, dilation=3)
            ...
            for fil in listdir(src_path):
                if fil.endswith('jpg') or fil.endswith('png') or fil.endswith('jpeg') or fil.endswith('JPG') or fil.endswith('PNG') or fil.endswith('JPEG'):
                    try:
                        shutil.copy(os.path.join(src_path, fil), os.path.join(out_path,'decensor_input_original', fil)) # DCP is compatible with original jpg input.
                    except Exception as e:
                        print("ERROR in hentAI_detection: Mosaic copy to decensor_input_original failed!", fil, e)
            detect_instance.run_on_folder(input_folder=src_path, output_folder=out_folder, is_video=False, is_mosaic=True, dilation=3)
    else:
