...
class HentaiDataset(utils.Dataset):
    ...
    def load_mask(self, image_id):
        ...
        np_class_id = np.asarray(
            class_id
        )  # std lists dont have shape, so convert to np
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "command", metavar="<command>", help="'train' or 'inference' supported"
    )
    parser.add_argument(
        "--dataset",
        required=False,
    ...
    parser.add_argument(
        "--sources",
        required=False,
        metavar="path to images input_images_folder or video input_images_folder",
        help="Source input_images_folder to run on",
    )
    parser.add_argument(
        "--dtype",
        required=False,
    ...
    parser.add_argument(
        ...
        help="Enter path to your DeepCreamPy input_images_folder",
    )
    ...
    if args.command == "train":
        ...
        assert (
            args.dtype
        ), "Argument --dtype required for type of inference. Use -h for help"
    ...
    if args.command == "train":
        ...
        out_folder = os.path.join(out_path, "decensor_input", "")
        print("Starting inference")
        detect_instance = Detector(
            weights_path=weights_path
        )  # declare instance and load weights
        ...
        if args.dtype == "esrgan":
            ...
            # First copy over all original files into input_original input_images_folder
            for fil in listdir(src_path):
                if (
                    ...
                    try:
                        shutil.copy(
                            os.path.join(src_path, fil),
                            os.path.join(out_path, "decensor_input_original", fil),
                        )  # DCP is compatible with original jpg input.
