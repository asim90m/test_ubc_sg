...
# You can change those input_images_folder paths
...
def rand_color():
    ...
    color_var = 0
    if random.random() >= 0.5:  # half chance for white
        r = random.randrange(239, 255)
        g = r
        b = r
        var_amnt = random.randrange(
            1, 25
        )  # ~half chance of >16, which is guaranteed overflow, so variation doesnt happen too much.
        if r + var_amnt > 255:
        ...
        var_amnt = random.randrange(
            10, 70
        )  # same idea, part of range will guarantee no variation
        if r + var_amnt > 50:
            var_amnt = 0
            print("canceled")
            return r, g, b  # Early cancellation
...
def pixelate(image, ratio, mosaic_kernel, interp):
    ...
    temp = cv2.resize(
        image, (w, h), interpolation=interp
    )  # cv2.INTER_AREA, cv2.INTER_CUBIC, cv2.INTER_LANCZOS4, cv2.INTER_NEAREST, cv2.INTER_LINEAR
...
def draw_angled_rec(x0, y0, width, height, angle, img, color, img_x, img_y, mode, q):
    ...
    while quantity > 0:
        ...
        bl = [
            int(x0 - a * height_s - b * width_s),
            int(y0 + b * height_s - a * width_s),
        ]
        ul = [
            int(x0 + a * height_s - b * width_s),
            int(y0 - b * height_s - a * width_s),
    ...
    if q == 2:
        ...
        conturs, _ = cv2.findContours(
            card, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE
...
with open("example.csv", "w", newline="", encoding="utf-8") as f_output:  # CSV
    csv_output = csv.writer(
        f_output, quoting=csv.QUOTE_NONE, quotechar="", delimiter=",", escapechar=" "
    )  # CSV
    csv_output.writerow(
        [
            "filename",
        ...
        try:
            while True:
                ...
                if all_regions == []:
                    ...
                    os.rename(
                        f, f.replace(rootdir, rejected, 1)
                    )  # to remove file from input to rejected
                ...
                mosaic_kernel = int(
                    random.triangular(
                        int(min(x * 0.01, y * 0.01)),
                        int(min(x * 0.2, y * 0.2)),
                        int(min(x * 0.0625, y * 0.0625)),
                    )
                )  # mosaic resolution 0.5%~33% with
                ...
                for idx, _ in enumerate(NudeNet_regions):
                    csv_output.writerow(
                        [
                            ...
                            idx,
                            '"{""name"":""polygon""',
                            '""all_points_x"":' + str(output1x[idx]),
                            '""all_points_y"":' + str(output1y[idx]) + '}"',
                            '"{""censor"":""bar""}"',
                        ]
                    )  # CSV
                points = []
                ...
                for region in all_regions:
                    ...
                    score = random.triangular(
                        area * 0.15, area * 0.3
                    )  # maximal area for rectangles
                    i = 0
                    while score >= area * 0.03:
                        if len_x >= len_y:  # decide the longest side
                            # print("vertical bar")
                            mode = "vertical"
                            thickness = random.triangular(
                                len_x * 0.03, len_x * 0.15
                            )  # thickness of the bar
                            wideness = random.triangular(
                                len_y * 0.3, len_y * 0.75
                            )  # wideness of the bar
                            angle = 0  # axis
                            bar_x = int(random.uniform(min_x, max_x))  # random bar_x
                            bar_y = int(
                                random.triangular(min_y, max_y)
                            )  # , min_y+(max_y-min_y)/2-wideness/2))    #random bar_y
                            # print(bar_x, bar_y)
                            ...
                            mode = "horizontal"
                            thickness = random.triangular(
                                len_y * 0.03, len_y * 0.15
                            )  # thickness of the bar
                            wideness = random.triangular(
                                len_x * 0.3, len_x * 0.75
                            )  # wideness of the bar
                            angle = 90  # axis
                            bar_x = int(
                                random.triangular(min_x, max_x)
                            )  # , min_x+(max_x-min_x)/2-wideness/2))    #random bar_x
                        ...
                        if thickness * wideness <= score + area * 0.02:
                            rotate = random.randint(
                                angle - 15, angle + 15
                            )  # random angle within 15% from axis
                            # print(rotate)
                            if rotate < 0:
                                rotate += 360
                            if not any(check in comp_area for check in comp_array):
                                ...
                                score -= (
                                    thickness * wideness
                                )  # subtract last rectangle from maximal area for rectangles
                ...
                f = f.replace(
                    os.path.splitext(f)[0], os.path.splitext(f)[0] + "_bar", 1
                )
                os.makedirs(os.path.dirname(f), exist_ok=True)
                cv2.imwrite(
                    "temp_out.png", img_rgb
                )  # still a hack for non-unicode names
                os.replace("temp_out.png", f)
