...
class Detector:
    ...
    def __init__(self, weights_path):
        ...
        self.model = modellib.MaskRCNN(
            mode="inference", config=self.config, model_dir=DEFAULT_LOGS_DIR
        )
        try:
            self.out_path = os.path.join(os.path.abspath("."), "AI/ESR_temp/ESR_out/")
            self.out_path2 = os.path.join(os.path.abspath("."), "AI/ESR_temp/ESR_out2/")
            self.temp_path = os.path.join(os.path.abspath("."), "AI/ESR_temp/temp/")
            self.temp_path2 = os.path.join(os.path.abspath("."), "AI/ESR_temp/temp2/")
            self.fin_path = os.path.join(os.path.abspath("."), "AI/ESR_output/")
        except:
            print(
                "ERROR in Detector init: Cannot find ESR_out or some dec_input within."
            )
            return
        # Create esrgan instance for detector instance
        try:
            self.esr_model_path = os.path.join(
                os.path.abspath("."), "4x_FatalPixels_340000_G.pth"
            )
        except:
            print(
    ...
    def load_weights(self):
        ...
        try:
            ...
            print(
                "ERROR in load_weights: Model Load. Ensure you have your weights.h5 file!",
                end=" ",
            )
            print("""hi""")
            print(e)
    ...
    def resize_GAN(self, img_path, img_name, is_video=False):
        ...
        if is_video is False:
            ...
            try:
                ...
                print(
                    "ERROR in resize_GAN: Image read. Skipping. image_path=", img_path
                )
                print(e)
                return
            ...
            if (
                granularity < 10
            ):  # TODO: implement adaptive granularity by weighted changes
            ...
            self.esrgan_instance.run_esrgan(
                test_img_folder=file_name,
            ...
            while success:
                ...
                if success:
                    ...
                    file_name = (
                        self.temp_path + img_name[:-4] + ".png"
                    )  # need to save a sequence of pngs for TGAN operation
                    skimage.io.imsave(
                        file_name, mini_img
                    ...
                    gan_img_path = (
                        self.out_path + img_name[:-4] + str(count).zfill(6) + ".png"
                    )
                    self.esrgan_instance.run_esrgan(
                        test_img_folder=file_name,
                        out_filename=gan_img_path,
    ...
    def ESRGAN(self, img_path, img_name, is_video=False):
        ...
        if is_video == False:
            ...
            try:
                ...
                print(
                    "ERROR in ESRGAN: Image write. Skipping. image_path=", img_path, e
                )
        else:
            # Video capture
            try:
                ...
                vwriter = VideoWriter(
                    file_name, VideoWriter_fourcc(*"mp4v"), fps, (width, height)
                )
            except Exception as e:
            ...
            try:
                ...
                ffmpeg.concat(in_video, in_audio, v=1, a=1).output(
                    img_path[:-4] + "_decen_audio.mp4", video_bitrate="11M"
                ).run()
            except Exception as e:
                print(
    ...
    # ESRGAN input_images_folder running function
    def run_ESRGAN(self, in_path=None, is_video=False, force_jpg=True):
        ...
        self.esrgan_instance = ColabESRGAN.test.esrgan(
    ...
    def video_create(self, image_path=None, dcp_path=""):
        ...
        for img in img_list:
            ...
            image = skimage.io.imread(
                img
            )  # Should be no alpha channel in created image
        ...
        try:
            ...
            ffmpeg.concat(in_video, in_audio, v=1, a=1).output(
                video_path[:-4] + "_decen_audio.mp4", video_bitrate="11M"
            ).run()
        except Exception as e:
            print(
    ...
    # save path and orig video input_images_folder are both paths, but orig video input_images_folder is for original mosaics to be saved.
    # fname = filename.
    # image_path = path of input file, image or video
    def detect_and_cover(
        self,
        image_path=None,
        fname=None,
        save_path="",
        ...
        if is_video:
            ...
            while success:
                ...
                if success:
                    ...
                    im_name = fname[
                        :-4
                    ]  # if we get this far, we definitely have a .mp4. Remove that, add count and .png ending
                    file_name = (
                        orig_video_folder + im_name + str(count).zfill(6) + ".png"
                    )  # NOTE Should be adequite for having 10^6 frames, which is more than enough for even 30 mintues total.
    ...
    def run_on_folder(
        ...
        if is_video == True:
            ...
            for file in os.listdir(str(input_folder)):
                ...
                if (
                    file_s.endswith("mp4")
                    or file_s.endswith("MP4")
                    or file_s.endswith("avi")
                ):
            ...
            # obtain inputs from the input input_images_folder
            img_list = []
            for file in os.listdir(str(input_folder)):
                file_s = str(file)
                try:
                    if (
                        file_s.endswith(".png")
                        or file_s.endswith(".PNG")
                        or file_s.endswith(".jpg")
                        or file_s.endswith(".JPG")
                    ):
                        path = os.path.join(input_folder, file_s)
                        img_list.append((path, file_s))
                except:
                    print("ERROR in run_on_folder: File parsing. file=", file_s)

            # save run detection with outputs to output input_images_folder
            for img_path, img_name in img_list:
                star = time.perf_counter()
                self.detect_and_cover(
                    img_path,
                    ...
                    is_mosaic=is_mosaic,
                    dilation=dilation,
                )  # sending force_jpg for debugging
                fin = time.perf_counter()
                total_time = fin - star
                print(
                    "Detection on image",
...
"""if __name__ == '__main__':
    ...
    parser.add_argument('--imagedir', required=True,
                        metavar="path to image input_images_folder",
    ...
    print('running ESRGAN on in and out input_images_folder')
