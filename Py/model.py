...
class InpaintNN:
    ...
    def check_model_file(self):
        if not os.path.exists(self.bar_model_name) or not os.path.exists(
    ...
    def build_model(self):
        ...
        I_co = mm.decoder(
            vec_en, self.input_height, self.input_height, reuse=False, name="G_de"
        )
        I_ge = mm.decoder(
            vec_con, self.input_height, self.input_height, reuse=True, name="G_de"
        )
        ...
        var_G = [
            ...
            if v.name.startswith("G_en")
            or v.name.startswith("G_de")
        ...
        if self.is_mosaic:
            ...
            Restore.restore(
                self.sess, tf.train.latest_checkpoint(self.mosaic_checkpoint_name)
            )
        else:
            Restore = tf.train.import_meta_graph(self.bar_model_name)
            Restore.restore(
                self.sess, tf.train.latest_checkpoint(self.bar_checkpoint_name)
            )

    def predict(self, censored, unused, mask):
        img_sample = self.sess.run(
            self.image_result,
            feed_dict={self.X: censored, self.Y: unused, self.MASK: mask},
        )
