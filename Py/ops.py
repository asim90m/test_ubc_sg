...
def ff_mask(
    size, b_zise, maxLen, maxWid, maxAng, maxNum, maxVer, minLen=20, minWid=15, minVer=5
):
                ...
                for x in range(wlx, whx):
                    for y in range(wly, why):
                        d = abs(
                            ...
                            - endY * startX
                            + startY * endX
                        ) / mt.sqrt((startY - endY) ** 2 + (startX - endX) ** 2)
...
def ff_mask_batch(
    size, b_size, maxLen, maxWid, maxAng, maxNum, maxVer, minLen=20, minWid=15, minVer=5
):
    mask = None
    temp = ff_mask(
        size,
