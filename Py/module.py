...
def conv_nn(input, dims1, dims2, size1, size2, k_size=3):
    ...
    L1 = layers.conv2d(
        pp, dims1, [k_size, k_size], stride=[1, 1], padding="VALID", activation_fn=None
    )
    L1 = tf.nn.elu(L1)

    pp = tf.pad(L1, [[0, 0], [1, 1], [1, 1], [0, 0]], "REFLECT")
    L2 = layers.conv2d(
        pp, dims2, [k_size, k_size], stride=[1, 1], padding="VALID", activation_fn=None
    )
    L2 = tf.nn.elu(L2)
...
def encoder(input, reuse, name):
    with tf.variable_scope(name):
        ...
        CL1 = layers.conv2d(
            p, 32, [5, 5], stride=[1, 1], padding="VALID", activation_fn=None
        )
        CL1 = tf.nn.elu(CL1)  # 256 256 32

        p = tf.pad(CL1, [[0, 0], [1, 1], [1, 1], [0, 0]], "REFLECT")
        CL2 = layers.conv2d(
            p, 64, [3, 3], stride=[2, 2], padding="VALID", activation_fn=None
        )
        CL2 = tf.nn.elu(CL2)  # 128 128 64

        p = tf.pad(CL2, [[0, 0], [1, 1], [1, 1], [0, 0]], "REFLECT")
        CL3 = layers.conv2d(
            p, 64, [3, 3], stride=[1, 1], padding="VALID", activation_fn=None
        )
        CL3 = tf.nn.elu(CL3)  # 128 128 64

        p = tf.pad(CL3, [[0, 0], [1, 1], [1, 1], [0, 0]], "REFLECT")
        CL4 = layers.conv2d(
            p, 128, [3, 3], stride=[2, 2], padding="VALID", activation_fn=None
        )
        CL4 = tf.nn.elu(CL4)  # 64 64 128

        p = tf.pad(CL4, [[0, 0], [1, 1], [1, 1], [0, 0]], "REFLECT")
        CL5 = layers.conv2d(
            p, 128, [3, 3], stride=[1, 1], padding="VALID", activation_fn=None
        )
        CL5 = tf.nn.elu(CL5)  # 64 64 128

        p = tf.pad(CL5, [[0, 0], [1, 1], [1, 1], [0, 0]], "REFLECT")
        CL6 = layers.conv2d(
            p, 256, [3, 3], stride=[2, 2], padding="VALID", activation_fn=None
        )
        CL6 = tf.nn.elu(CL6)  # 32 32 128

        p = tf.pad(CL6, [[0, 0], [2, 2], [2, 2], [0, 0]], "REFLECT")
        DCL1 = layers.conv2d(
            p, 256, [3, 3], rate=2, stride=[1, 1], padding="VALID", activation_fn=None
        )
        DCL1 = tf.nn.elu(DCL1)
        p = tf.pad(DCL1, [[0, 0], [4, 4], [4, 4], [0, 0]], "REFLECT")
        DCL2 = layers.conv2d(
            p, 256, [3, 3], rate=4, stride=[1, 1], padding="VALID", activation_fn=None
        )
        DCL2 = tf.nn.elu(DCL2)
        p = tf.pad(DCL2, [[0, 0], [8, 8], [8, 8], [0, 0]], "REFLECT")
        DCL3 = layers.conv2d(
            p, 256, [3, 3], rate=8, stride=[1, 1], padding="VALID", activation_fn=None
        )
        DCL3 = tf.nn.elu(DCL3)
        p = tf.pad(DCL3, [[0, 0], [16, 16], [16, 16], [0, 0]], "REFLECT")
        DCL4 = layers.conv2d(
            p, 256, [3, 3], rate=16, stride=[1, 1], padding="VALID", activation_fn=None
        )
...
def decoder(input, size1, size2, reuse, name):
    with tf.variable_scope(name):
        ...
        LL2 = layers.conv2d(
            DL4, 3, [3, 3], stride=[1, 1], padding="SAME", activation_fn=None
        )  # 256 256 3
...
def discriminator_G(input, reuse, name):
    with tf.variable_scope(name):
        ...
        L2 = layers.conv2d(
            p, 128, [5, 5], stride=2, padding="VALID", activation_fn=None
        )
        ...
        L3 = layers.conv2d(
            p, 256, [5, 5], stride=2, padding="VALID", activation_fn=None
        )
...
def discriminator_L(input, reuse, name):
    with tf.variable_scope(name):
        ...
        p = tf.pad(L3, [[0, 0], [2, 2], [2, 2], [0, 0]], "REFLECT")
        L4 = layers.conv2d(
            p, 512, [5, 5], stride=2, padding="VALID", activation_fn=None
        )
...
def contextual_block(bg_in, fg_in, mask, k_size, lamda, name, stride=1):
    with tf.variable_scope(name):
        ...
        patch1 = tf.extract_image_patches(
        ...
        patch2 = tf.extract_image_patches(
            fg_in, [1, k_size, k_size, 1], [1, 1, 1, 1], [1, 1, 1, 1], "SAME"
        )
        ACL = []
...
def contextual_block_cs(bg_in, fg_in, mask, k_size, lamda, name, stride=1):
    with tf.variable_scope(name):
        ...
        for ib in range(b):
            k1 = patch1[ib, :, :, :, :]
            k2 = k1 / tf.sqrt(
                tf.reduce_sum(tf.square(k1), axis=2, keepdims=True) + 1e-16
            )
            k1 = tf.reshape(k1, (k_size, k_size, dims, c))
            ww = patch2[ib, :, :, :]
            ft = ww / tf.sqrt(
                tf.reduce_sum(tf.square(ww), axis=2, keepdims=True) + 1e-16
            )
